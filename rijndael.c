...
static unsigned char galoisFieldMultiplier(unsigned char a) {
  if (a & 0x80) {
...
static void mix_single_column(unsigned char* block, int row) {
  ...
  unsigned char u = a[0];
  a[0] ^= t ^ galoisFieldMultiplier(a[0] ^ a[1]);
  a[1] ^= t ^ galoisFieldMultiplier(a[1] ^ a[2]);
  a[2] ^= t ^ galoisFieldMultiplier(a[2] ^ a[3]);
  a[3] ^= t ^ galoisFieldMultiplier(a[3] ^ u);
  block[row * 4 + 0] = a[0];
  block[row * 4 + 1] = a[1];
  block[row * 4 + 2] = a[2];
  block[row * 4 + 3] = a[3];
